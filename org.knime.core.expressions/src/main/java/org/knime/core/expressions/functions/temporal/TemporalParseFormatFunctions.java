...
public final class TemporalParseFormatFunctions {
     ...
     * @throws ExpressionEvaluationException
     */
    private static <T extends TemporalAccessor> boolean checkIfFormatIsUnderSpecified(final String format,
     ...
     * @throws ExpressionEvaluationException
     */
    private static <T extends TemporalAccessor> boolean checkIfFormatIsOverspecified(final String format,
        final TemporalQuery<T> query) throws ExpressionEvaluationException {
        var formatter = checkDateTimeFormat(format);
    ...
    private static Computer parseDateImpl(final Arguments<Computer> args) {
        var formatComputer = (StringComputer)args.get(FORMAT_ARG, StringComputer.ofConstant(null));
        ...
        ComputerResultSupplier<Optional<LocalDate>> valueSupplier = ctx -> {
            ...
            try {
                ...
                if (checkIfFormatIsUnderSpecified(format, LocalDate::from)) {
                    // if the format is underspecified, we give a special warning
                    ctx.addWarning("Format '%s' is underspecified for LOCAL_DATE.".formatted(dateString));
                } else {
    ...
    private static Computer parseTimeImpl(final Arguments<Computer> args) {
        ...
        ComputerResultSupplier<Optional<LocalTime>> valueSupplier = ctx -> {
            ...
            try {
                ...
                if (checkIfFormatIsUnderSpecified(format, LocalTime::from)) {
                    // if the format is underspecified, we give a special warning
                    ctx.addWarning("Format '%s' is underspecified for LOCAL_TIME.".formatted(timeString));
                } else {
    ...
    private static Computer parseDateTimeImpl(final Arguments<Computer> args) {
        ...
        ComputerResultSupplier<Optional<LocalDateTime>> valueSupplier = ctx -> {
            ...
            try {
                ...
                if (checkIfFormatIsUnderSpecified(format, LocalDateTime::from)) {
                    // if the format is underspecified, we give a special warning
                    ctx.addWarning("Format '%s' is underspecified for LOCAL_DATE_TIME.".formatted(dateTimeString));
                } else {
                    ctx.addWarning(
    ...
    private static Computer parseZonedDateTime(final Arguments<Computer> args) {
        ...
        ComputerResultSupplier<Optional<ZonedDateTime>> valueSupplier = ctx -> {
            ...
            try {
                ...
                if (checkIfFormatIsUnderSpecified(format, ZonedDateTime::from)) {
                    // if the format is underspecified, we give a special warning
                    ctx.addWarning("Format '%s' is underspecified for ZONED_DATE_TIME.".formatted(zonedDateTimeString));
                    return Optional.empty();
                } else {
        ...
        return args -> {
            ...
            ComputerResultSupplier<Optional<String>> valueSupplier = ctx -> {
                ...
                try {
                    ...
                    if (checkIfFormatIsOverspecified(formatString, query)) {
    ...
    public static final ExpressionFunction FORMAT_DATE = functionBuilder() //
        ...
        .examples("""
                ...
                * `format_date(parse_date("1970-01-01"), "yyyy-MM-dd HH")` causes an error (overspecified format)
                """) //
    ...
    public static final ExpressionFunction FORMAT_TIME = functionBuilder() //
        ...
        .examples("""
                ...
                * `format_time(parse_time("12:34"), "invalid format")` causes an error (invalid format)
    ...
    public static final ExpressionFunction FORMAT_DATE_TIME = functionBuilder() //
        ...
        .examples("""
                ...
                * `format_datetime($["my_datetime_col"], "dd/MM/yyyy HH:mm:ss VV")` causes an error \
