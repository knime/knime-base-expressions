...
/**
 * Node dialog implementation of the Generic ECharts View.
 *
 * @author Benjamin Wilhelm, KNIME GmbH, Berlin, Germany
 */
@SuppressWarnings("restriction")
final class JsonEditorNodeDialog implements NodeDialog {

    private JsonEditorScriptingService m_scriptingService;

    JsonEditorNodeDialog() {
        m_scriptingService = new JsonEditorScriptingService();
    }

    @Override
    public Page getPage() {
        return Page //
            .builder(JsonEditorNodeDialog.class, "js-src/dist", "json-editor.html") //
    ...
    public NodeSettingsService getNodeSettingsService() {
        var workflowControl = Optional.ofNullable(NodeContext.getContext()) //
        ...
        var initialData = GenericInitialDataBuilder.createDefaultInitialDataBuilder(NodeContext.getContext()) //
            ...
            .addDataSupplier("flowVariables", () -> {
                var flowVariables = Optional.ofNullable(workflowControl.getFlowObjectStack()) //
                    ...
                    .orElseGet(List::of);
                // TODO template for insertion text
                return InputOutputModel.flowVariables().subItems(flowVariables, Object::toString, type -> true).build();
            });

        return new ScriptingNodeSettingsService(JsonEditorSettings::new, initialData);
    }
