...
import { DEFAULT_ROW_FILTER_NODE_INITIAL_DATA } from "@/__mocks__/mock-data";
...
const mockedScriptingService = vi.hoisted(() => ({
  ...
  getInitialData: vi.fn(() =>
    Promise.resolve(DEFAULT_ROW_FILTER_NODE_INITIAL_DATA),
  ),
...
describe("ExpressionsRowFilterApp.vue", () => {
  ...
  it("renders function catalog with data from service", async () => {
    ...
    expect(functionCatalog.props("functionCatalogData")).toEqual(
      DEFAULT_ROW_FILTER_NODE_INITIAL_DATA.functionCatalog,
    );
  });
});
