...
import { DEFAULT_INITIAL_DATA } from "@/__mocks__/scripting-service-browser-mock";
...
describe("App.vue", () => {
  ...
  it("renders function catalog with data from service", async () => {
    ...
    expect(functionCatalog.props("functionCatalogData")).toEqual(
      DEFAULT_INITIAL_DATA.functionCatalog as FunctionCatalogData,
    );
  });
  ...
  it("renders column selector and check that it is updated when promises are resolved", async () => {
    ...
    expect(columnSelector.props("modelValue")).toEqual({
      createColumn: DEFAULT_INITIAL_DATA.settings.createdColumns[0],
      outputMode: "APPEND",
      replaceColumn: DEFAULT_INITIAL_DATA.settings.replacedColumns[0],
    });
  });
});
